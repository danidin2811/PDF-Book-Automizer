...
def append_to_existing_toc(input_pdf_path, output_pdf_path, new_toc_entries):
    ...
    if not new_toc_entries:
        print_red("Error: No TOC entries provided to append.")
        return False
    ...
    try:
        ...
        # Recursive function to clone existing bookmarks
        def copy_outlines(outlines, parent_item=None):
            last_added = None
            for item in outlines:
                if isinstance(item, list):
                    copy_outlines(item, parent_item=last_added)
                else:
        ...
        # Append new bookmarks from Gemini/CSV
        bookmark_stack = {}
        for entry in new_toc_entries:
            ...
            if page_index < 0 or page_index >= len(reader.pages):
                print(f"  [SKIP] Page {entry['page']} of {title} is out of range.")
                continue
            ...
            # Clean up stack for lower levels
        ...
        print_red(f"Permission Denied: Ensure {output_pdf_path} is closed!")
        return False

    except Exception as e:
        print_red(f"Unexpected TOC Error: {e}")
        return False
